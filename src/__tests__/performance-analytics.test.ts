...
// Import type definitions for mock objects
import type { PerformanceService } from '@/services/analytics/PerformanceService';
import type { RealtimeService } from '@/services/RealtimeService';
import type { MetricsCalculator } from '@/services/analytics/metrics-calculator';

// Mock the actual imports
jest.mock('@/services/analytics/PerformanceService', () => ({
...
jest.mock('@/services/analytics/metrics-calculator', () => ({
  MetricsCalculator: jest.fn().mockImplementation(() => ({
    calculateRevenueMetrics: jest.fn()
  }))
}));

// Mock implementations are now handled in the import section above
...
describe('RHY_063 Performance Analytics - Cycle 1: Core Functionality', () => {
  ...
  beforeEach(() => {
    ...
    performanceService = {
      ...
      getSystemPerformance: jest.fn(),
      getResourceMetrics: jest.fn()
    } as jest.Mocked<PerformanceService>;
    ...
    metricsCalculator = {
      calculateRevenueMetrics: jest.fn()
    } as jest.Mocked<MetricsCalculator>;
  ...
  describe('Executive Dashboard Generation', () => {
    it('should generate executive dashboard with all required KPIs', async () => {
      ...
      performanceService.getExecutiveDashboard.mockResolvedValue(mockDashboardData);

      const result = await performanceService.getExecutiveDashboard('US_WEST', {
        period: '30d',
        includeAlerts: true,
        includeTrends: true
      });
      ...
      Object.values(result.kpis).forEach(kpi => {
    ...
    it('should handle warehouse access validation', async () => {
      ...
      performanceService.validateWarehouseAccess = jest.fn().mockResolvedValue(mockSupplierAccess);

      const accessResult = await performanceService.validateWarehouseAccess('supplier-123', 'US_WEST');
    ...
    it('should generate proper error handling for invalid requests', async () => {
      performanceService.getExecutiveDashboard.mockRejectedValue(
        new Error('Warehouse not found: INVALID_WAREHOUSE')
      );

      await expect(
        performanceService.getExecutiveDashboard('INVALID_WAREHOUSE', { period: '30d' })
  ...
  describe('Real-Time Report Generation', () => {
    it('should generate real-time performance reports', async () => {
      ...
      performanceService.generateRealTimeReport.mockResolvedValue(mockRealtimeReport);

      const result = await performanceService.generateRealTimeReport('US_WEST');
    ...
    it('should handle real-time data streaming', async () => {
      ...
      realtimeService.getRealtimeMetrics.mockResolvedValue({
      ...
      const result = await realtimeService.getRealtimeMetrics('US_WEST', '1h');
  ...
  describe('Predictive Analytics', () => {
    it('should generate AI-powered forecasts', async () => {
      ...
      performanceService.getPredictiveAnalytics.mockResolvedValue(mockPredictiveAnalytics);

      const result = await performanceService.getPredictiveAnalytics('US_WEST', { 
        horizon: 30,
        includeRisks: true,
      ...
      Object.values(result.forecasts).forEach(forecast => {
    ...
    it('should validate prediction confidence levels', async () => {
      ...
      performanceService.getPredictiveAnalytics.mockResolvedValue(mockAnalytics);

      const result = await performanceService.getPredictiveAnalytics('US_WEST', { horizon: 7 });

      // Validate confidence levels are within acceptable range (75-95%)
      result.forecasts.efficiency.predictions.forEach(prediction => {
  ...
  describe('Export Functionality', () => {
    it('should export executive reports in multiple formats', async () => {
      ...
      performanceService.exportExecutiveReport.mockResolvedValue(mockExportResult);

      const result = await performanceService.exportExecutiveReport('US_WEST', {
        format: 'ALL',
        period: '30d',
        includeCharts: true,
        includeRawData: true
    ...
    it('should handle large dataset exports efficiently', async () => {
      ...
      performanceService.exportLargeDataset.mockResolvedValue(mockLargeDatasetExport);

      const result = await performanceService.exportLargeDataset('US_WEST', {
  ...
  describe('Performance Monitoring', () => {
    it('should track system performance metrics', async () => {
      ...
      performanceService.getSystemPerformance.mockResolvedValue(mockPerformanceMetrics);

      const result = await performanceService.getSystemPerformance();
    ...
    it('should validate memory usage and resource consumption', async () => {
      ...
      performanceService.getResourceMetrics.mockResolvedValue(mockResourceMetrics);

      const result = await performanceService.getResourceMetrics();
  ...
  describe('Data Integrity and Validation', () => {
    it('should validate data consistency across metrics', async () => {
      ...
      metricsCalculator.calculateRevenueMetrics.mockResolvedValue({
        ...
        netMargin: 15.0,
        revenueByProduct: [
          { productId: '6Ah', name: '6Ah FlexVolt Battery', revenue: 660000, percentage: 30 },
          { productId: '9Ah', name: '9Ah FlexVolt Battery', revenue: 990000, percentage: 45 },
          { productId: '15Ah', name: '15Ah FlexVolt Battery', revenue: 550000, percentage: 25 }
        ],
        revenueByChannel: [
      ...
      const revenueResult = await metricsCalculator.calculateRevenueMetrics({
      ...
      const totalPercentage = revenueResult.revenueByProduct.reduce(
        (sum, product) => sum + product.percentage, 0
      );
    ...
    it('should handle edge cases and data anomalies', async () => {
      ...
      const zeroResult = await metricsCalculator.calculateRevenueMetrics({
        start: new Date(),
        end: new Date()
      });
      ...
      const negativeResult = await metricsCalculator.calculateRevenueMetrics({
        start: new Date(),
        end: new Date()
      });
